...
namespace ApsMotionControl
{
    ...
    public class Vision
    {
        ...
        private bool CAM_LOAD_CHECK = false;
        ...
        public void AllocMilCCdBuffer(int nIndex, int nimageWidth, int nimageHeight) //Laon
        {
            ...
            for (i = 0; i < 3; i++)
            {
                ...
                unsafe
                {
                    ...
                    //IntPtr MilImagePtrIntPtr = CompressedBufferId;
                    //byte* MilImageAddr = (byte*)MilImagePtrIntPtr;


                    // 데이터를 복사
                    //Marshal.Copy(m_pImgBuff[i], 0, bufferPtr, pitch * Hight); // 메모리로 데이터 복사
                    //byte* buffer = (byte*)bufferPtr.ToPointer(); // 포인터로 캐스팅하여 접근
                    //
                    //m_pImgBuff[nIndex][i] = (unsigned char*)GlobalAlloc(GMEM_ZEROINIT, nSizeX * nSizeY);
                    //IntPtr bufferPtr = Marshal.AllocHGlobal(bufferSize); // 1024바이트 크기의 메모리 할당
                    // Marshal.Copy(m_pImgBuff[nIndex][i], 0, bufferPtr, bufferSize);




                    //Marshal.Copy(m_pImgBuff[i], 0, bufferPtr, bufferSize);
                    //((MyData*)this.myPtr.ToPointer())[index];
                    //Marshal.FreeCoTaskMem(ptr);

                    //m_pImgBuff[i] = Marshal.AllocHGlobal(ptr);
                    //IntPtr hglobal = Marshal.AllocHGlobal(100);
                    //IntPtr hglobal = Marshal.AllocHGlobal(nimageWidth * nimageHeight);
                    //Copy(byte[] source, int startIndex, IntPtr destination, int length);

                    //Marshal.Copy(m_pImgBuff[i], 0, hglobal, 3);
                    //Marshal.FreeHGlobal(hglobal);
                    // m_pImgBuff[nIndex, i] = (byte*)GlobalAlloc(0x0040, (UIntPtr) nimageWidth * nimageHeight);//GMEM_ZEROINIT
                    // 할당한 메모리를 해제
            ...
            MIL.MbufAllocColor(MilSystem, 3, Globalo.camControl.mCCdWindow.Width, Globalo.camControl.mCCdWindow.Height, (8 + MIL.M_UNSIGNED), MIL.M_IMAGE + MIL.M_PROC + MIL.M_DISP, ref m_MilSmallImage[0]);
